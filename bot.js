...
setInterval(() => {
  http.get(`http://la-music-001.glitch.me/`);
}, 280000);
...
const GUILDID = "658579369844801536"; //  تعديل مهم ايدي السيرفر
const CHANNELID = "659155996698083328"; // تعديل مهم ايدي الروم الصوتي
...
let cmds = {
  ...
  function voiceStay(guildid, channelid) {
    ...
    if (!channelid)
      throw new Error("Syntax: voiceStay function requires channelid");
  ...
  if (cmd === "play") {
    ...
    if (!voiceChannel)
      return msg.channel.send(
        `<a:692301521920016:680840184862081062> You must be listening in a voice channel to use that!`
      );
    const permissions = voiceChannel.permissionsFor(msg.client.user);
    if (!permissions.has("CONNECT")) {
      return msg.channel.send(
        `<a:692301521920016:680840184862081062> I can't join Your voiceChannel because i don't have ` +
          "`" +
          "`CONNECT`" +
          "`" +
          ` permission!`
    ...
    if (!permissions.has("SPEAK")) {
      return msg.channel.send(
        `<a:692301521920016:680840184862081062> I can't SPEAK in your voiceChannel because i don't have ` +
          "`" +
          "`SPEAK`" +
          "`" +
          ` permission!`
    ...
    voiceChannel.join();
    if (!args[0])
      return msg.channel.send(`**> K-MUSIC v1.6 Music Bot:
>    -play \`\`<song name>\`\`
>    -vol \`\`<volume>\`\`
>    -skip**`);
    ...
    async function handleVideo(video, msg, voiceChannel, playlist = false) {
      ...
      if (!serverQueue) {
        ...
        if (!args) return msg.channel.send("no results.");
          ...
          return msg.channel
            .send(
              "<a:81047588787867:680841628440723466> Loading... [`" +
                args +
                "`]"
            )
            .then(m => {
              setTimeout(() => {
                //<a:81047588787867:680841628440723466> Loading... [let]
                m.edit(
                  `<a:692301521920085:680840196526309469> Added **${song.title}**` +
                    "(` " +
                    song.duration +
                    ")`" +
    ...
    function play(guild, song) {
      ...
      if (serverQueue.repeating) {
        ...
        serverQueue.textChannel.send(
          "<a:692301521920085:680840196526309469> Added **" +
            song.title +
            "** (`" +
            song.duration +
            "`) to begin playing."
    ...
    return msg.channel.send(
      "<a:81047588787867:680841628440723466> The player has stopped and the queue has been cleared."
    );
  } else if (cmd === "skip") {
    ...
    if (req == 1) {
      msg.channel.send(
        "**<a:692301521920085:680840196526309469> Skipped **" + args
      );
    ...
    if (!queue.playing)
      return msg.channel.send(
        "<a:692301521920016:680840184862081062> There must be music playing to use that!"
      );
    ...
    msg.channel.send(
      "<a:692301521920085:680840196526309469> Paused " +
        args +
        ". **Type** `" +
        prefix +
        "resume` to unpause!"
    ...
    if (!queue)
      return msg.channel.send(
        "<a:692301521920085:680840196526309469> No music paused to resume."
      );
    ...
    msg.channel.send("<a:692301521920085:680840196526309469> Resumed.");
  } else if (cmd === "volume") {
    let queue = active.get(msg.guild.id);

    if (!queue || !queue.songs)
      return msg.channel.send(
        "<a:81047588787867:680841628440723466> There is no music playing to set volume."
      );
    ...
    if (!vCh || vCh !== msg.guild.me.voiceChannel)
      return msg.channel.send(
        "<a:81047588787867:680841628440723466> You are not in my voice channel"
      );
    ...
    if (isNaN(args[0]))
      return msg.channel.send(
        "<a:81047588787867:680841628440723466> Numbers only!"
      );
    ...
    if (queue.repeating) {
      ...
      return msg.channel.send(
        "<a:692301521920085:680840196526309469> **Repeating Mode** (`False`)"
      );
    } else {
      queue.repeating = true;
      return msg.channel.send(
        "<a:692301521920085:680840196526309469> **Repeating Mode** (`True`)"
      );
    }
...
client
  ...
  .catch(err => console.log("قد لايوجد توكن او التوكن متوقف . [KAHv1.6]"));
